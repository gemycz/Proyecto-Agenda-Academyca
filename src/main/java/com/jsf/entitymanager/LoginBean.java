...
public class LoginBean {
	...
	public String validateLoginCredentials() {
		...
		EntityManager entityMgrObj = Persistence.createEntityManagerFactory(PERSISTENCE_UNIT_NAME)
				.createEntityManager();
		Query queryObj = entityMgrObj
				.createQuery("SELECT u FROM UserEntityManager u WHERE u.login = :login AND u.password = :password");
		...
		try {
			...
			if ((userResultSetObj != null) && (userName.equalsIgnoreCase(userResultSetObj.getLogin())
			...
			FacesContext.getCurrentInstance().addMessage("loginForm:loginName",
					new FacesMessage("Usuario o Contraseña es Incorrecta"));
		}
